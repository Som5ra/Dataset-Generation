...
class Generator:
  ...
  def save_data(self, path: Path, objects: List[bproc.types.MeshObject], data: Dict):
    ...
    bproc.writer.write_coco_annotations(os.path.join(str(path.absolute()), 'coco_data'),
    ...
    # for frame in range(bproc.utility.num_frames()):
    #   with h5py.File(str(path.absolute()) + f'/{frame}.hdf5') as hdf5:
    #     rgb = np.array(hdf5['colors'])
    #     img = Image.fromarray(rgb)
    #     img.save((str(path.absolute()) + f'/{frame}.hdf5').replace('.hdf5', '.png'))
    #   break
  ...
  def create_lights(self, scene_size: float, target_objects: List[bproc.types.MeshObject]) -> List[bproc.types.Light]:
    ...
    total_light_count = np.random.randint(min_count, max_count + 1)
    
    lights = []
    def basic_light_sampling(light: bproc.types.Light) -> None:
      loc = np.random.uniform([-scene_size / 2, -scene_size / 2, -scene_size / 2], [scene_size / 2, scene_size / 2, scene_size / 2])
      random_selected_wall_dim = np.random.choice([0, 1, 2], size = 2)
      random_selected_wall_pos = np.random.choice([-scene_size / 2, scene_size / 2], size = 2)
      loc[random_selected_wall_dim[0]] = random_selected_wall_pos[0]
      loc[random_selected_wall_dim[1]] = random_selected_wall_pos[1]
    ...
    def spec_light_sampling(light: bproc.types.Light, dim = 0, pn = -1) -> None:
      loc = [0, 0, 0]
      loc[dim] = -1 * scene_size / 2
      # loc = np.random.uniform([-scene_size / 2, -scene_size / 2, -scene_size / 2], [scene_size / 2, scene_size / 2, scene_size / 2])
      # random_selected_wall_dim = np.random.choice([0, 1, 2], size = 2)
      # random_selected_wall_pos = np.random.choice([-scene_size / 2, scene_size / 2], size = 2)
      # loc[random_selected_wall_dim[0]] = random_selected_wall_pos[0]
      ...
      light.set_location(loc)
      light.set_energy(np.random.uniform(min_intensity, max_intensity))
      light.set_color(np.random.uniform(0.8, 1.0, size=3))
    '''
    arealight_count = np.random.randint(0, total_light_count + 1)
    arealight_count = max(arealight_count, total_light_count // 3)
    for _ in range(arealight_count):
      ...
      lights.append(light)
    '''
    for _ in range(total_light_count + 1):
      light = bproc.types.Light('POINT')
      updown = -1 if _ % 2 else 1
      # spec_light_sampling(light, _ % 3, updown)
      basic_light_sampling(light)
      # Point towards a target object
      looked_at_obj = np.random.choice(len(target_objects))
      looked_at_obj: bproc.types.MeshObject = target_objects[looked_at_obj]
      ...
      light.set_rotation_mat(R)
      lights.append(light)
    print("[create_lights]: created", len(lights), "lights")
    return lights
  ...
  def create_target_objects(self) -> List[bproc.types.MeshObject]:
    ...
    print(f"[create_target_objects]: creating {len(objects)} objects")
    for obj_i, object in enumerate(objects):
      object.set_cp("category_id", obj_i + 1)
      object.hide(False)
      if scale_noise > 0.0:
  ...
  def create_scene(self):
    ...
    def relocate_objects_with_limit(objects: List[bproc.types.MeshObject], limit_anchor: bproc.types.MeshObject, max_distance: float):
      for obj in objects:
        loc = bproc.sampler.sphere(limit_anchor, max_distance, 'INTERIOR')
        obj.set_location(loc)
    ...
    anchor = np.random.uniform([-size / 3, -size / 3, -size / 3], [size / 3, size / 3, size / 3])
    max_distance = 0.0
    for obj in objects:
      max_distance = max(max_distance, object_bb_length(obj))
    relocate_objects_with_limit(objects, anchor, max_distance * 1.5)
    ...
    print(f"[create_scene]: created scene with {len(objects)} objects, {len(distractors)} distractors, {len(lights)} lights")
    return scene


  def add_bias2_to_rotation_mat(self, ori_rotation_euler: np.ndarray, angle: float) -> np.ndarray:
    random_angle = np.random.uniform(-angle, angle, size=3)
    resampled_rotation_euler = np.array(Euler(ori_rotation_euler + random_angle).to_matrix())
    return resampled_rotation_euler
  # Example usage in sample_camera_poses
  def sample_camera_poses(self, scene: Scene) -> None:
      ...
      for frame in range(images_per_scene):
        ...
        while not good and tries < 1000:
          random_obj_idx = np.random.choice(len(scene.target_objects))
          object = scene.target_objects[random_obj_idx]
          ...
          while not good and tries_distance < 100:
            # print(f"[sample_camera_poses] tries_distance: {tries_distance}")
            # add 20 degrees randomness to the forward vector
            selected_object_rotation_euler = object.get_rotation_euler()
            # biased_rotation_euler = self.add_bias2_to_rotation_mat(selected_object_rotation_euler, np.pi / 4)
            biased_rotation_euler = self.add_bias2_to_rotation_mat(selected_object_rotation_euler, self.json_config['camera']['angle_bias_face_to_object'])
            # random_angle = np.random.uniform(-np.pi / 9, np.pi / 9, size=3)
            # random_camera_forward_vec = np.array(Euler(object.get_rotation_euler() + random_angle).to_matrix())
            forward_vec_y = np.array([biased_rotation_euler[0][1], biased_rotation_euler[1][1], biased_rotation_euler[2][1]])

            location = object.get_location() + forward_vec_y * distance * object_size
            # if location is outside the room, reject it
            is_inside = lambda loc: np.all(np.logical_and(loc < hs, loc > -hs))
            if not is_inside(location):
              tries += 1
              tries_distance += 1
              continue
            # location = bproc.sampler.sphere(object.get_location(), distance * object_size, 'SURFACE')
            _inplane_rot = self.json_config['camera']['inplane_rot']
            rotation_matrix = bproc.camera.rotation_from_forward_vec(poi - location, inplane_rot=np.random.uniform(-_inplane_rot, _inplane_rot))
            ...
            if object in bproc.camera.visible_objects(cam2world_matrix, 10) and bproc.camera.perform_obstacle_in_view_check(cam2world_matrix, {'min': focal_length + clip_start}, bvh):
              good = True
              for idx, obj in enumerate(scene.target_objects):
                if idx == random_obj_idx:
                  continue
                biased_rotation = self.add_bias2_to_rotation_mat(selected_object_rotation_euler, np.pi / 9)
                obj.set_rotation_mat(biased_rotation)
            else:
              good = False
            tries += 1
            tries_distance += 1
